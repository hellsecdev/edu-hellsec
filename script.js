...
const i18n = {
  ru: {
    ...
    'דגש': 'Акцент',
    'AI ככלי עבודה, עם הבנה': 'AI как рабочий инструмент, с пониманием',
    ...
    'תוצרים שאפשר לפתוח ולהציג': 'Работы, которые можно открыть и показать',
    'בסיס להחלטה אם להרחיב לקורס': 'Основа для решения о расширении в курс',
    'מתאים במיוחד כאשר': 'Особенно подходит, когда',
    'בית הספר רוצה להתחיל עם AI בצורה מסודרת, עם פעילות קצרה ותוצאה ברורה.': 'Школа хочет начать с AI аккуратно, с короткой практической активностью и понятным результатом.',
    ...
    'התלמידים בודקים תשובות של AI ומציגים רק עבודה שהם מבינים.': 'Ученики проверяют ответы AI и показывают только работу, которую понимают.',
    ...
    'איך מתחילים בלי סיכון מיותר?': 'Как начать без лишнего риска?',
    'לא צריך לשנות מערכת לימודים שלמה. מתחילים מקבוצה אחת, רואים תוצרים אמיתיים, ואז מחליטים אם להרחיב.': 'Не нужно менять всю учебную систему. Начинаем с одной группы, смотрим реальные результаты, затем решаем, расширять ли формат.',
    'שיחת התאמה קצרה': 'Короткая проверка формата',
    'מבינים גיל, רמת כיתה, אישורים, מחשבים ומטרה לימודית.': 'Понимаем возраст, класс, разрешения, компьютеры и учебную цель.',
    'פיילוט לקבוצה אחת': 'Пилот для одной группы',
    'מפגש של 2 עד 3 שעות עם תוצר שאפשר לפתוח בסוף.': '2-3 часа с результатом, который можно открыть в конце.',
    'תוצרים והחלטה': 'Результаты и решение',
    'בודקים קישורים, הסברים של תלמידים והתאמה להמשך.': 'Проверяем ссылки, объяснения учеников и подходит ли продолжение.',
    'מה בית הספר מקבל?': 'Что получает школа?',
    'כניסה מסודרת לעבודה עם AI בכיתה': 'Спокойный и понятный вход в работу с AI в классе',
    'תוצרים שאפשר לפתוח ולהציג': 'Работы, которые можно открыть и показать',
    'שפה ברורה להנהלה, מורים והורים': 'Понятный язык для администрации, учителей и родителей',
    'בסיס להחלטה אם להמשיך לקורס קצר או מלא': 'Основа для решения: продолжать ли в короткий или полный курс',
    'דוגמה מעשית': 'Практический пример',
    'כך נראה תוצר שאפשר להציג': 'Так выглядит результат, который можно показать',
    'התלמיד מתחיל ממשפט פשוט: יש לי רעיון ואני רוצה להפוך אותו לעמוד. בסוף הפיילוט יש אתר קצר, קוד שמור וקישור שאפשר לפתוח.': 'Ученик начинает с простой фразы: у меня есть идея, я хочу превратить её в страницу. В конце пилота есть короткий сайт, сохранённый код и ссылка, которую можно открыть.',
    'לפני': 'До',
    "רעיון לא מסודר במחברת או בצ'אט": 'Несобранная идея в тетради или чате',
    'אחרי': 'После',
    'עמוד אישי עם הסבר, תמונה, קישורים ופרסום': 'Личная страница с объяснением, изображением, ссылками и публикацией',
    'עבודה עם AI שמתאימה לסביבה בית ספרית': 'Работа с AI, подходящая для школьной среды',
    'המסגרת פשוטה: לא מפרסמים מידע רגיש, עובדים רק עם כלים מאושרים, וכל תלמיד מציג רק תוצר שהוא יודע להסביר.': 'Правила простые: не публикуем чувствительные данные, работаем только с одобренными инструментами, ученик показывает только то, что может объяснить.',
    'אין פרסום של מידע אישי רגיש. תמונות ותוכן נבחרים לפי כללי בית הספר.': 'Чувствительные личные данные не публикуются. Изображения и материалы выбираются по правилам школы.',
    'כלים מאושרים בלבד': 'Только одобренные инструменты',
    'שימוש ב AI, GitHub ופרסום נעשה לפי אישור ומדיניות בית הספר.': 'AI, GitHub и публикация используются только по разрешению и политике школы.',
    'הבנה לפני הצגה': 'Понимание перед презентацией',
    'התלמיד חייב לדעת להסביר מה הקוד עושה ומה השתנה בעזרת AI.': 'Ученик должен уметь объяснить, что делает код и что изменилось с помощью AI.',
    'מסגרת מונחית': 'Управляемый формат',
    'עבודה בקצב שמתאים לכיתה, עם יעדים, בדיקה ותיקון בשלבים.': 'Работа в темпе класса, с целями, проверкой и исправлением по шагам.',
    'מפתח ועובד עם מערכות תוכנה, AI, סייבר ואוטומציה. התוכנית מועברת כמו עבודה מעשית: רעיון, קוד, בדיקה, תיקון, פרסום והסבר.': 'Разрабатывает и работает с ПО, AI, кибербезопасностью и автоматизацией. Программа практическая: идея, код, проверка, исправление, публикация и объяснение.',
    'נבדוק ב 10 דקות אם זה מתאים לבית הספר': 'Проверим за 10 минут, подходит ли это школе',
    'שלחו הודעה קצרה עם כיתה, מספר תלמידים, זמינות מחשבים והאם רוצים פיילוט או קורס. משם אפשר להציע התחלה פשוטה וברורה.': 'Отправьте короткое сообщение: класс, количество учеников, есть ли компьютеры и нужен ли пилот или курс. После этого можно предложить простой понятный старт.',
    'מה לכתוב?': 'Что написать?',
    'איזו כיתה או שכבה': 'Какой класс или параллель',
    'כמה תלמידים בקבוצה': 'Сколько учеников в группе',
    'האם יש מחשבים ואינטרנט': 'Есть ли компьютеры и интернет',
    'פיילוט חד פעמי או קורס': 'Разовый пилот или курс',
    'בדיקת התאמה ב WhatsApp': 'Проверить в WhatsApp',
    'חזרה למעלה': 'Наверх'
  },
  en: {
    ...
    'דגש': 'Focus',
    'AI ככלי עבודה, עם הבנה': 'AI as a working tool, with understanding',
    ...
    'תוצרים שאפשר לפתוח ולהציג': 'Student work that can be opened and shown',
    'בסיס להחלטה אם להרחיב לקורס': 'A basis for deciding whether to expand into a course',
    'מתאים במיוחד כאשר': 'Best suited when',
    'בית הספר רוצה להתחיל עם AI בצורה מסודרת, עם פעילות קצרה ותוצאה ברורה.': 'The school wants to start with AI carefully, through a short practical activity with a clear result.',
    ...
    'התלמידים בודקים תשובות של AI ומציגים רק עבודה שהם מבינים.': 'Students check AI answers and present only work they understand.',
    ...
    'איך מתחילים בלי סיכון מיותר?': 'How to start without unnecessary risk?',
    'לא צריך לשנות מערכת לימודים שלמה. מתחילים מקבוצה אחת, רואים תוצרים אמיתיים, ואז מחליטים אם להרחיב.': 'No need to change the whole curriculum. Start with one group, see real outcomes, then decide whether to expand.',
    'שיחת התאמה קצרה': 'Short fit check',
    'מבינים גיל, רמת כיתה, אישורים, מחשבים ומטרה לימודית.': 'Understand age, class level, approvals, computers and the learning goal.',
    'מפגש של 2 עד 3 שעות עם תוצר שאפשר לפתוח בסוף.': 'A 2 to 3 hour session with an outcome students can open at the end.',
    'תוצרים והחלטה': 'Outcomes and decision',
    'בודקים קישורים, הסברים של תלמידים והתאמה להמשך.': 'Review links, student explanations and whether it fits a next step.',
    'מה בית הספר מקבל?': 'What does the school get?',
    'כניסה מסודרת לעבודה עם AI בכיתה': 'A clear first step into AI work in class',
    'תוצרים שאפשר לפתוח ולהציג': 'Student work that can be opened and shown',
    'שפה ברורה להנהלה, מורים והורים': 'Clear language for leadership, teachers and parents',
    'בסיס להחלטה אם להמשיך לקורס קצר או מלא': 'A basis for deciding whether to continue to a short or full course',
    'דוגמה מעשית': 'Practical example',
    'כך נראה תוצר שאפשר להציג': 'An outcome students can actually present',
    'התלמיד מתחיל ממשפט פשוט: יש לי רעיון ואני רוצה להפוך אותו לעמוד. בסוף הפיילוט יש אתר קצר, קוד שמור וקישור שאפשר לפתוח.': 'A student starts with a simple sentence: I have an idea and want to turn it into a page. At the end of the pilot, they have a short website, saved code and a link they can open.',
    'לפני': 'Before',
    "רעיון לא מסודר במחברת או בצ\'אט": 'An unstructured idea in a notebook or chat',
    'אחרי': 'After',
    'עמוד אישי עם הסבר, תמונה, קישורים ופרסום': 'A personal page with explanation, image, links and publication',
    'עבודה עם AI שמתאימה לסביבה בית ספרית': 'AI work that fits a school environment',
    'המסגרת פשוטה: לא מפרסמים מידע רגיש, עובדים רק עם כלים מאושרים, וכל תלמיד מציג רק תוצר שהוא יודע להסביר.': 'The rules are simple: no sensitive personal data, approved tools only, and students present only work they can explain.',
    'אין פרסום של מידע אישי רגיש. תמונות ותוכן נבחרים לפי כללי בית הספר.': 'No sensitive personal information is published. Images and content follow school rules.',
    'כלים מאושרים בלבד': 'Approved tools only',
    'שימוש ב AI, GitHub ופרסום נעשה לפי אישור ומדיניות בית הספר.': 'AI, GitHub and publication are used according to school approval and policy.',
    'הבנה לפני הצגה': 'Understanding before presentation',
    'התלמיד חייב לדעת להסביר מה הקוד עושה ומה השתנה בעזרת AI.': 'The student must be able to explain what the code does and what changed with AI.',
    'מסגרת מונחית': 'Guided framework',
    'עבודה בקצב שמתאים לכיתה, עם יעדים, בדיקה ותיקון בשלבים.': 'Work at a class-appropriate pace, with goals, checks and step-by-step fixing.',
    'מפתח ועובד עם מערכות תוכנה, AI, סייבר ואוטומציה. התוכנית מועברת כמו עבודה מעשית: רעיון, קוד, בדיקה, תיקון, פרסום והסבר.': 'Builds and works with software systems, AI, cybersecurity and automation. The program is practical: idea, code, testing, fixing, publishing and explanation.',
    'נבדוק ב 10 דקות אם זה מתאים לבית הספר': 'Check in 10 minutes if this fits your school',
    'שלחו הודעה קצרה עם כיתה, מספר תלמידים, זמינות מחשבים והאם רוצים פיילוט או קורס. משם אפשר להציע התחלה פשוטה וברורה.': 'Send a short message with the grade, group size, computer availability and whether you want a pilot or course. From there, we can suggest a simple starting format.',
    'מה לכתוב?': 'What to send?',
    'איזו כיתה או שכבה': 'Which class or grade',
    'כמה תלמידים בקבוצה': 'How many students in the group',
    'האם יש מחשבים ואינטרנט': 'Whether computers and internet are available',
    'פיילוט חד פעמי או קורס': 'One-time pilot or course',
